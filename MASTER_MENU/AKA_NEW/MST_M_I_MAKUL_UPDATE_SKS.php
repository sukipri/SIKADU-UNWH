	...
	$kdsks=@$sql_escape($_GET['kdsks']);
	 $sks = $call_q("$call_sel sks where idsks='$kdsks'");
	$skss = $call_fas($sks);
	...
	$call_q("update sks set iddosen='$dosen',idsemester='$semester',idruang='$kdruang',idkurikulum='$judul',jumlah='$jumlah',hari='$hari',jam='$jam',idkelas='$kelas',kuota='$kuota',app='$app',batas_dosen='$batas' where idsks='$kdsks' ");
	$call_q("update krs set jumlah='$jumlah' where idsks='$kdsks'");
	...
	<form action="" method="post" name="form1" onSubmit="MM_validateForm('judul','','R','jumlah','','RisNum');return document.MM_returnValue">
	  ...
	  echo"<option value=$smm1[idsemester]>$smm1[semester] &nbsp; $thh[ajaran] &nbsp; $smm1[ajaran]</option>";
		...
		<tr valign="middle">
		  ...
		  $skskr = $call_q("$sl idkurikulum,judul,juduleng from kurikulum where idkurikulum='$skss[idkurikulum]'");
